...
public class OracleTableFactory implements TableFactory<OracleTable, InputStream> {

	public static Object[] getInfos(InputStream inputStream) throws IOException {
		final Object[] result = new Object[2];
		final Table<OracleColumn> table = new OracleTable();
		result[0] = table;
	 ...
	 */
	@Override
	public OracleTable buildTable(InputStream input) throws NumberFormatException, IOException {
		final OracleTable table = new OracleTable();
		...
		try {
			...
			for (String line; (line = reader.readLine()) != null;) {
				...
				column.setLocation(Integer.valueOf(args[0]));
				column.setName(args[1]);
				if (args[2].indexOf("(") != -1) column.setTypeByStr(args[2].substring(0, args[2].indexOf("(")).trim());
				else column.setTypeByStr(args[2]);
				table.addColumn(column);
			}
		} finally {
