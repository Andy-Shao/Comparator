...
public class OnlyFileAHasElement<T extends Item<?>> implements DifferentResultVisitable<T> {
	private volatile List<T> value;
    private Map<String, Object> otherValues;
	
	public OnlyFileAHasElement(List<T> list) {
		this(list, new HashMap<String, Object>());
    }
	
	public OnlyFileAHasElement(List<T> list, Map<String, Object> map) {
		this.value = list;
